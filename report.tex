...
\subsubsection*{Constructing the Complement DFA}
...
% explain using the union of turing machines, and other TM analogies

Decide which hold in general:
\begin{enumerate}
  \item If \(L_1,L_2\) are decidable then \(L_1\cup L_2\) is decidable. % chain the TMs together so if the first one exepts it goes on to run the second 1.
  \item If \(L\) is decidable then \(\overline L\) is decidable.
  \item If \(L\) is decidable then \(L^*\) is decidable.
  \item If \(L_1,L_2\) are r.e.\ then \(L_1\cup L_2\) is r.e. % consider copying the language to a second tape and running both machines in parallel, if one exepts than it r.e
  \item If \(L\) is r.e.\ then \(\overline L\) is r.e.
  \item If \(L\) is r.e.\ then \(L^*\) is r.e.
\end{enumerate}
\textbf{Answer.}
\begin{enumerate} % elaborate for the final report.
  \item True.
  \item True.
  \item True.
  \item True.
